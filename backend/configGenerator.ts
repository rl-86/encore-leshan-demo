...
class LeshanBulkConfigurer {
  ...
  private loadApiToken(): string {
    ...
    for (const envPath of envPaths) {
      if (fs.existsSync(envPath)) {
        ...
        for (const line of lines) {
          if (line.startsWith('NEXT_PUBLIC_API_TOKEN=')) {
            const token = line.split('=', 2)[1]?.trim().replace(/['"]/g, '');
            console.log('[INFO] API token loaded successfully');
            return token;
          }
        }
      }
    ...
    console.log(
      '[WARNING] Could not find .env.local file or NEXT_PUBLIC_API_TOKEN'
    );
  ...
  private async configureClient(
    ...
    try {
      ...
      const bsUrl = `${this.baseUrl}/api/bsclients/${endpoint}`;
  ...
  async bulkConfigure(endpoints: string[]) {
    console.log(`\n${'='.repeat(60)}`);
...
async function main() {
  ...
  const endpoints = Array.from(
    { length: config.count },
    (_, i) =>
      `${config.devicePrefix}${String(config.startNumber + i).padStart(
        config.paddingLength,
        '0'
      )}`
  );

  await configurer.bulkConfigure(endpoints);
}
